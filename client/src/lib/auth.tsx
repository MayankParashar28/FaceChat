      ...
      if (firebaseUser) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            // Prefer server-provided avatar; it is unique and persisted in MongoDB
            if (dbUser && dbUser.avatar) {
              setUser((prev) => prev ? { ...prev, photoURL: dbUser.avatar } : prev);
            } else if (!firebaseUser.photoURL) {
              // Fallback to deterministic DiceBear avatar based on uid
              const fallback = `https://api.dicebear.com/9.x/adventurer/svg?seed=${encodeURIComponent(firebaseUser.uid)}`;
              setUser((prev) => prev ? { ...prev, photoURL: fallback } : prev);
            }
